...
public class AdminGifts : MonoBehaviour
{
    public static AdminGifts instance;
    ...
    public void ButtonLoadGifts()
    {
        PhotonManager.Inst.peer.SendOperation(
            (byte)Request.LoadGifts,
            new Dictionary<byte, object>(),
            PhotonManager.Inst.sendOptions);
    }

    public void ShowGifts(ParameterDictionary data)
    {
        Debug.Log("show gifts");
        
        UiHelper.ClearContainer(adminGiftsUiContainer);
                
        var giftsData = (Dictionary<int, object>)data[(byte)Params.gifts];

        Debug.Log(giftsData.Count);

        foreach (var ed in giftsData)
        {
            var extraData = (Dictionary<byte, object>)ed.Value;

            AddAdminGiftUi(extraData);
        }
    }

    [SerializeField] private AdminGiftUi adminGiftUiPrefab;
    [SerializeField] private Transform adminGiftsUiContainer;
    private void AddAdminGiftUi(Dictionary<byte, object> data)
    {
        var newAdminExtraUi = Instantiate(adminGiftUiPrefab);
        UiHelper.AssignObjectToContainer(newAdminExtraUi.gameObject, adminGiftsUiContainer);
