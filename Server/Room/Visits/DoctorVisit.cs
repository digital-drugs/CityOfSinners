...
namespace Mafia_Server
{
    class DoctorVisit : RoleVisit
    {
        ...
        public void Setup()
        {
            ...
            doctor.SetActiveAtNight(doctor.targetPlayer != null);

            //проверяем, что цель жива на начало ночи
            if (doctor.targetPlayer != null && !doctor.targetPlayer.isLive()) doctor.targetPlayer = null;
        }
        ...
        private Doctor GetRole()
        {
            Doctor role = null;

            if (doctor.oldRole != null)
            {
                role = (Doctor)doctor.oldRole;
            }
            else
            {
                role = (Doctor)doctor.playerRole;
            }
